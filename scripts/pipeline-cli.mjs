 ...
 *   node scripts/pipeline-cli.mjs context <text>          # pipeline-context.md에 수동 append
 */
...
const CONTEXT_PATH = join(ROOT, "logs", "pipeline-context.md");
...
const ADVANCE_VALIDATORS = {
  ...
  report: (state) => {
    const wc = state.phases["worktree-clean"];
    if (wc.status !== "completed")
      return ["worktree-clean phase가 완료되지 않았습니다."];
    if (!existsSync(CONTEXT_PATH))
      return [
        "pipeline-context.md가 없습니다. Phase 완료 시 --context를 사용했는지 확인하세요.",
      ];
    return [];
  },
};
...
function appendContext(phase, text) {
  ...
  const entry = `\n[${timestamp}] **${phase}** — ${text}\n`;
  appendFileSync(CONTEXT_PATH, entry, "utf-8");
}

function initContext(pipelineId) {
  const dir = dirname(CONTEXT_PATH);
  if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
  const header = `# Pipeline Context Journal\n\nPipeline: ${pipelineId}\nStarted: ${new Date().toISOString()}\n`;
  writeFileSync(CONTEXT_PATH, header, "utf-8");
}
...
function cmdComplete(phase, completeArgs) {
  const state = loadState();
  if (!state) {
    console.log("❌ 파이프라인 상태 파일이 없습니다.");
    process.exit(1);
  ...
  // Parse --context and JSON from args array
  let contextText = null;
  let outputJson = null;

  if (completeArgs && completeArgs.length > 0) {
    const ctxIdx = completeArgs.indexOf("--context");
    if (ctxIdx !== -1 && ctxIdx + 1 < completeArgs.length) {
      contextText = completeArgs[ctxIdx + 1];
      // Remove --context and its value, keep the rest
      const remaining = [
        ...completeArgs.slice(0, ctxIdx),
        ...completeArgs.slice(ctxIdx + 2),
      ];
      if (remaining.length > 0) outputJson = remaining.join(" ");
    } else {
      outputJson = completeArgs.join(" ");
    }
  }
  ...
  if (outputJson) {
    try {
      ...
      console.log(`받은 값: ${outputJson.slice(0, 200)}`);
      process.exit(1);
    }
  }
  ...
  if (contextText) {
    appendContext(phase, contextText);
    console.log(`📝 컨텍스트 저널에 기록됨`);
  } else if (!OPTIONAL_PHASES.has(phase) && phase !== "worktree-clean") {
    console.log(
      `⚠️  --context 누락 — 보고서 품질을 위해 Phase 요약을 기록하세요:`,
    );
    console.log(`   node scripts/pipeline-cli.mjs context "Phase 요약 텍스트"`);
  }
...
function cmdContext(text) {
  const state = loadState();
  const phase = state?.currentPhase || "manual";
  appendContext(phase, text);
  console.log(`📝 컨텍스트 저널에 기록됨 [${phase}]`);
}
...
switch (command) {
  ...
  case "complete":
    if (!args[0]) {
      console.log(
        'Usage: pipeline-cli.mjs complete <phase> [--context "text"] [output-json]',
      );
      process.exit(1);
    }
  ...
  case "context":
    if (!args[0]) {
      console.log('Usage: pipeline-cli.mjs context "텍스트"');
      process.exit(1);
    }
...
Commands:
  ...
  complete <phase> [opts] [json]  phase 완료 + output 기록
    --context "text"            컨텍스트 저널에 Phase 요약 기록
  context "text"               컨텍스트 저널에 수동 기록
